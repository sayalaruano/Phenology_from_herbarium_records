...
df_synonyms <- data_frame(df["Scientific_names"][0:50,])
colnames(df_synonyms) <- "Scientific_names"

# Create an index for the original names
index = 1

for(i in df["Scientific_names"][1:50,]) {
...
write_csv(df_synonyms, "List_50_species_phenology_proj_UDLA2022_with_synonyms.csv")
